...
router.get('/', authRequired, (req, res) => {
  ...
  query += ' WHERE p.owner_user_id = $1';

  if (search) {
    query += ` AND (LOWER(p.item_name) LIKE $${params.length + 1} OR LOWER(p.hsn) LIKE $${params.length + 1} OR LOWER(c.name) LIKE $${params.length + 1})`;
...
router.post('/', authRequired, async (req, res) => {
  ...
  const sql = `
    INSERT INTO products (owner_user_id, hsn, item_name, item_price, category_id, gst_percent, discount_percent, manufactured_date, expiry_date, stock_updated_date, stock_present, threshold_stock)
...
router.delete('/:id', authRequired, (req, res) => {
  ...
  pool.query('DELETE FROM products WHERE id = $1 AND owner_user_id = $2', [id, req.user.id], (err, result) => {
    if (err) {
      console.error('Database error', err);
      return res.status(500).json({ message: 'Server error' });
    }
...
router.put('/:id', authRequired, async (req, res) => {
  ...
  try {
    ...
    `, [Number(itemPrice) || 0, categoryId, Number(gst) || 0, Number(discount) || 0, mfd || null, expiryDate || null, stockUpdatedDate || null, Number(stockPresent) || 0, Number(thresholdStock) || 0, id, req.user.id]);
